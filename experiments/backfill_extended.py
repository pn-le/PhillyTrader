"""backfill_extended.py — EXTENDED IEX 1-minute history backfill (2024-01-01 .. 2026-06-03).

Attempts to push the cached 1-min history further back to capture more market regimes
than the original 190-day window (2025-09-02 ..). Probing confirmed the free IEX tier
serves 1-min bars back to at least 2024-01. This script re-fetches each universe symbol
(SPY included) over 2024-01-01 .. 2026-06-03, RTH-filters (America/New_York [09:30,16:00)
on bar START), sorts, de-duplicates on `start`, and OVERWRITES the existing parquet cache
with the longer history. Schema is byte-compatible with the original cache (same columns,
same tz-aware NY `start`, harness.load_bars reads it unchanged).

Pure research / read-only market-data fetch. Places ZERO orders.

Run:  cd /Users/pnle/Desktop/alpaca-cli && python3 -m experiments.backfill_extended
"""
...
def main():
    ...
    for sym in UNIVERSE:
        ...
        print(f"{sym}: {len(df)} bars, {n_days} days, {first.date()} -> {last.date()}", flush=True)

    (RESULTS_DIR / "backfill_extended_coverage.json").write_text(json.dumps(coverage, indent=2))
    print("\nCoverage written to", RESULTS_DIR / "backfill_extended_coverage.json", flush=True)
    return coverage
